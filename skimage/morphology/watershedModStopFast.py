...
from . import _watershedModStopFast
from ..util import crop, regular_seeds


def _validate_inputs(image, markers, mask, triggerLabels, velocityLabels):
    ...
    if triggerLabels is not None and triggerLabels.shape != image.shape:
        raise ValueError("`triggerLabels` must have same shape as `image`")
    if triggerLabels is None:
        # Use a complete `False` mask if none is provided
        triggerLabels = np.zeros(image.shape, int)
...
def watershedModStopFast(image, markers, connectivity=1, offset=None, mask=None,
              triggerLabels=None, velocityLabels=None, compactness=0, watershed_line=False):
    ...
    """
    image, markers, mask, triggerLabels, velocityLabels = _validate_inputs(image, markers, mask, triggerLabels, velocityLabels)
    ...
    triggerLabels = np.pad(triggerLabels, pad_width, mode='constant').ravel()
    velocityLabels = np.pad(velocityLabels, pad_width, mode='constant').ravel()
    ...
    while(True):
        findLabel += 1
        idx = np.flatnonzero(velocityLabels==findLabel)
        if len(idx)==0:
            break
        if len(idx) > maxOccurrence:
            maxOccurrence = len(idx)

    #Create the array of indices
    connectedIndices = np.ones((np.max(velocityLabels), maxOccurrence), dtype=np.int32)*-1
    ...
    while(True):
        ...
        if len(idx) == 0:
            ...
            connectedIndices[findLabel-1][0:len(idx)] = idx
    
    "Ther first indices are"
    print(connectedIndices[:,0])
    ...
    print(triggerLabels.shape)
    print(velocityLabels.shape)

    _watershedModStopFast.watershed_raveled(image.ravel(),
                                 marker_locations, flat_neighborhood,
                                 mask, triggerLabels, velocityLabels, connectedIndices, image_strides, compactness,
                                 output.ravel(),
                                 watershed_line)
