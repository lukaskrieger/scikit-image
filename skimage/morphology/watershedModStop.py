...
from . import _watershedModStop
from ..util import crop, regular_seeds


def _validate_inputs(image, markers, mask, stoppingMask):
    ...
    if stoppingMask is not None and stoppingMask.shape != image.shape:
        raise ValueError("`stoppingMask` must have same shape as `image`")
    if stoppingMask is None:
        # Use a complete `False` mask if none is provided
        stoppingMask = np.zeros(image.shape, bool)
    return (image.astype(np.float64),
            markers.astype(np.int32),
            mask.astype(np.int8),
            stoppingMask.astype(np.int8))
...
def watershedModStop(image, markers, connectivity=1, offset=None, mask=None,
              stoppingMask=None, compactness=0, watershed_line=False):
    ...
    """
    image, markers, mask, stoppingMask = _validate_inputs(image, markers, mask, stoppingMask)
    ...
    stoppingMask = np.pad(stoppingMask, pad_width, mode='constant').ravel()
    ...
    _watershedModStop.watershed_raveled(image.ravel(),
                                 marker_locations, flat_neighborhood,
                                 mask, stoppingMask, image_strides, compactness,
                                 output.ravel(),
                                 watershed_line)
