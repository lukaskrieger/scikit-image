...
from . import _watershedFlowLines
from . import binary_dilation
from ..util import crop, regular_seeds


def _validate_inputs(image, markers, mask, velocityLabels):
    ...
    if velocityLabels is not None and velocityLabels.shape != image.shape:
        raise ValueError("`velocityLabels` must have same shape as `image`")
    if velocityLabels is None:
        # Use a complete `False` mask if none is provided
        velocityLabels = np.zeros(image.shape, bool)
    return (image.astype(np.float64),
            markers.astype(np.int32),
            mask.astype(np.int8),
            velocityLabels.astype(np.int32))
...
def watershedFlowLines(image, markers, connectivity=1, offset=None, mask=None, velocityLabels=None, compactness=0, watershed_line=False):
    ...
    """
    image, markers, mask, velocityLabels = _validate_inputs(image, markers, mask, velocityLabels)
    ...
    #ravel velocity labels later
    velocityLabels = np.pad(velocityLabels, pad_width, mode='constant')
    output = np.pad(markers, pad_width, mode='constant')

    #Find border pixels
    connectedBorderIndicesList = []
    connectedIndicesList = []
    print(velocityLabels.dtype)
    numberOfLabels = np.max(velocityLabels.astype(np.int))
    print("Number of different velocity labels", numberOfLabels)
    print("Ndim vel_labels", velocityLabels.shape)
    print(velocityLabels.dtype)
    maxConnectedBorder = 0
    maxConnected = 0
    for num in range(1,numberOfLabels+1):
        print(num+1,"/",numberOfLabels)
        #Get connected
        label_mask = (velocityLabels==num)
        connectingTo = np.flatnonzero(label_mask)
        
        #Get border
        dilated = binary_dilation(label_mask)
        connectingToBorder = np.flatnonzero(dilated != label_mask)

        if len(connectingTo) > maxConnected:
            maxConnected = len(connectingTo)

        if len(connectingToBorder) > maxConnectedBorder:
            maxConnectedBorder = len(connectingToBorder)

        connectedIndicesList.append(connectingTo)
        connectedBorderIndicesList.append(connectingToBorder)


    print("maxConnected labels", maxConnected)
    print("maxConnectedBorder labels", maxConnectedBorder)

    #Create the array of indices
    connectedIndices = np.ones((numberOfLabels, maxConnected), dtype=np.int32)*-1
    #Create the array of indices
    connectedBorderIndices = np.ones((numberOfLabels, maxConnectedBorder), dtype=np.int32)*-1

    #Fill the array with labels
    for i in range(0, numberOfLabels):
        connectedIndices[i,0:len(connectedIndicesList[i])] = connectedIndicesList[i]
        connectedBorderIndices[i,0:len(connectedBorderIndicesList[i])] = connectedBorderIndicesList[i]
    ...
    _watershedFlowLines.watershed_raveled(image.ravel(),
                                 marker_locations, flat_neighborhood,
                                 mask, velocityLabels.ravel(), connectedIndices, connectedBorderIndices, image_strides, compactness,
                                 output.ravel(),
                                 watershed_line)
