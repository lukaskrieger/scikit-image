...
def watershed_raveled(cnp.float64_t[::1] image,
                      ...
                      DTYPE_INT32_t[:,::1] connectedBorderIndices,
                      cnp.int32_t[::1] strides,
    ...
    cdef Py_ssize_t connectedIndex = 0
    cdef Py_ssize_t connectedBorderIndex = 0
    cdef DTYPE_INT32_t wsl_label = -1
    cdef int velocityLabel = 0
    cdef int outputAtSource = 0
    ...
    while hp.items > 0:
        ...
        if velocityLabels[elem.index]>0:
            
            # Reached a velocity point
            velocityLabel = velocityLabels[elem.index]
            print("Should only be called once for each vel label. This is", velocityLabel)
            
            outputAtSource = output[elem.source]
            ...
            #Go over indices of all connected border velocites
            for connectedBorderIndex in connectedBorderIndices[velocityLabel-1]:
                #Reached end point
                if connectedBorderIndex == -1:
                    break
                
                index = connectedBorderIndex
                ...
                if output[index]:
                    ...
                    if wsl:
                        ...
                        if output[index] != outputAtSource:
        ...
        elif velocityLabels[elem.index]==0:
